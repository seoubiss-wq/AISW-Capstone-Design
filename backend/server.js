...
async function buildRecommendationsFromPlaces(input, preferences = defaultPreferences()) {
  const baseQuery = `${input}`.trim();
  const results = await placesTextSearch(baseQuery);
  const maxDistanceKm = parseMaxDistanceKm(preferences.maxDistanceKm);
  const searchCenter = maxDistanceKm
  ...
  const food = results
    ...
    .filter(({ distanceKm }) =>
      maxDistanceKm && searchCenter && distanceKm != null
  ...
  if (!food.length) {
    if (maxDistanceKm && searchCenter) {
      throw new Error(
        `${maxDistanceKm}km ?대궡?먯꽌 議곌굔??留욌뒗 ?뚯떇?먯쓣 李얠? 紐삵뻽?듬땲?? 吏??챸????援ъ껜?곸쑝濡??낅젰?섍굅??嫄곕━ ?쒗븳???볧?蹂댁꽭??`,
      );
    }
    throw new Error("議곌굔??留욌뒗 ?뚯떇?먯쓣 李얠? 紐삵뻽?듬땲?? 寃?됱뼱瑜?議곌툑 諛붽퓭蹂댁꽭??");
  }

  return food.map(({ place, distanceKm }, index) => {
    const name = String(place.name || "").trim() || `異붿쿇 ${index + 1}`;
    const placeId = place.place_id;
    const photoRef = place.photos?.[0]?.photo_reference;

    const parts = [];
    if (distanceKm != null) parts.push(`검색 중심에서 ${distanceKm.toFixed(1)}km`);
    const reason = parts.join(" 쨌 ") || "Google 吏??寃??寃곌낵";
